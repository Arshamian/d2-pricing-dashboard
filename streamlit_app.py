...
with tabs[1]:
    ...
    if not comp_any.empty and comp_any["dep_month"].astype(str).str.len().eq(7).any():
        ...
        bar_metric = cc1.radio("Columns show", metrics, index=0, horizontal=True, key="dmg_bar")
        l1_default = "Win Rate %" if bar_metric != "Win Rate %" else "Avg Markup %"
        ...
        with st.expander("🔬 Focused view — filter by destination, window, comparison run date & booking dates"):
            ...
            bar2 = mc1.radio("Columns", metrics2, index=0, horizontal=True, key="fv_bar")
            l1d = "Win Rate %" if bar2 != "Win Rate %" else "Avg Markup %"
