...
with tabs[1]:
    ...
    # By travel month & destination — bookings, win rate and markup together
    res = dest_month_panels(v, bk_tx if HAS_TX else None)
    if res:
        figdm, dshown, ntot = res
        st.markdown("#### 📈 By Travel Month & Destination")
        cap = ("Bookings, win rate and average markup by travel (departure) month — one line per "
               "destination, shown separately. Markup panel marks the 7% floor and 15% ceiling; "
               "win-rate panel marks 50%.")
        if ntot > len(dshown):
