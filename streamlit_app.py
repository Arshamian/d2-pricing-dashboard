...
# ══════════════════════════════════════════════════════════════════════════════
# ACCESS CONTROL — password stored in the app's Secrets (Settings → Secrets)
# ══════════════════════════════════════════════════════════════════════════════
def require_password():
    """Gate the whole app behind a shared password held in Streamlit Secrets.
    On Streamlit Cloud: open the app ☰ menu → Settings → Secrets and add a line:
        password = "your-password-here"
    (locally, put the same line in .streamlit/secrets.toml)."""
    if st.session_state.get("auth_ok"):
        return
    try:
        configured = str(st.secrets["password"])
    except Exception:
        configured = ""

    st.markdown("### 🔒 D2 Pricing Intelligence Dashboard")
    if not configured:
        st.error("Password protection is on, but no password is configured yet. In Streamlit "
                 "Cloud, open the app's ☰ menu → **Settings → Secrets** and add:\n\n"
                 "```toml\npassword = \"choose-a-password\"\n```\n\nthen save (the app reruns).")
        st.stop()

    def _check():
        entered = st.session_state.get("pw_input", "")
        st.session_state["auth_ok"] = hmac.compare_digest(entered, configured)
        st.session_state.pop("pw_input", None)   # never keep the plaintext around

    st.text_input("Enter password to continue", type="password",
                  key="pw_input", on_change=_check)
    if st.session_state.get("auth_ok") is False:
        st.error("Incorrect password — please try again.")
    st.caption("Access is restricted. The password is managed by your admin in the app settings.")
    st.stop()
...
def init_db():
    ...
    conn.execute("""CREATE TABLE IF NOT EXISTS pricing(
        ...
        supplier TEXT, uploaded_at TEXT)""")
    # migrate older DBs that predate the supplier column
    try:
        have = [r[1] for r in conn.execute("PRAGMA table_info(pricing)").fetchall()]
        if "supplier" not in have:
            conn.execute("ALTER TABLE pricing ADD COLUMN supplier TEXT")
    ...
    # Active hotel offers (one row per offer; a giata may have several)
    conn.execute("""CREATE TABLE IF NOT EXISTS offers(
        giata TEXT, hotel_name TEXT, destination TEXT, offer_type_name TEXT,
        is_exclusive INTEGER, deal_status TEXT,
        book_from TEXT, book_to TEXT, travel_from TEXT, travel_to TEXT,
        details TEXT, uploaded_at TEXT)""")
    # Shared action log: which hotel has been actioned, against which pricing data date
    conn.execute("""CREATE TABLE IF NOT EXISTS completions(
        giata TEXT, file_date TEXT, hotel_name TEXT, destination TEXT,
        done INTEGER, action_taken TEXT, note TEXT, who TEXT, updated_at TEXT,
        PRIMARY KEY(giata, file_date))""")
    ...
    for sname, raw in sheets:
        ...
        for _, row in data.iterrows():
            try:
                ...
                supplier = merge_supplier(row.get(gc(8),""))
                ...
                all_rows.append({
                    ...
                    "priority_score":0.0,"supplier":supplier,"uploaded_at":datetime.now().isoformat(),
                })
            except: continue
    ...
    for r in rows:
        ...
        if c.fetchone() is None:
            c.execute("""INSERT INTO pricing(
                ...
                margin_flag,booking_tier,priority_score,supplier,uploaded_at)
                VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
                tuple(r[k] for k in [
                    "file_name","file_date","destination","competitor","giata","hotel_name","board",
                    "dep_date","dep_month","dep_window","nights","pp_price","current_margin","op_room",
...
def merge_supplier(name):
    """Merge supplier codes to their parent. The part after the dash is a CODE (merge to the
    parent) when it has no lowercase letters — e.g. 'OTS - AAEDXB7I06' → 'OTS',
    'Hilton - DXBBR' → 'Hilton', 'HotelBeds - 994856' → 'HotelBeds'. A named sub-brand keeps its
    full name (it contains lowercase) — e.g. 'Juniper - Travco' and 'Dolphin - HotelProductDatabase'
    stay intact."""
    s = str(name).strip()
    if not s or s.lower() in ("nan","none","-",""): return ""
    m = re.match(r"^(.*?)\s*-\s*(.+)$", s)
    if not m: return s
    parent, after = m.group(1).strip(), m.group(2).strip()
    if parent and not any(c.islower() for c in after):   # after-dash is a code → merge
        return parent
    return s
...
def load_offers_map():
    """Aggregate active hotel offers to one entry per giata that is LIVE today (deal active,
    today within the booking window, travel still ahead). Returns
    (dict giata->info, set of giatas with a live EXCLUSIVE offer). info = {exclusive, type_name}."""
    conn = get_conn()
    try: o = pd.read_sql_query("SELECT * FROM offers", conn)
    except Exception: o = pd.DataFrame()
    conn.close()
    if o.empty:
        return {}, set()
    o["giata"] = o["giata"].map(norm_giata)
    today = pd.Timestamp(datetime.now().date())
    def d(x): return pd.to_datetime(x, errors="coerce")
    bf, bt = d(o["book_from"]), d(o["book_to"])
    tt = d(o["travel_to"])
    active = o["deal_status"].fillna("").str.lower().eq("active") | o["deal_status"].isna() | o["deal_status"].eq("")
    live = active & (bf.isna() | (bf <= today)) & (bt.isna() | (bt >= today)) & (tt.isna() | (tt >= today))
    o = o[live].copy()
    if o.empty:
        return {}, set()
    o["is_exclusive"] = pd.to_numeric(o["is_exclusive"], errors="coerce").fillna(0).astype(int)
    info, excl = {}, set()
    for g, grp in o.groupby("giata"):
        is_excl = bool(grp["is_exclusive"].max())
        # prefer an exclusive offer's type name for the label
        pref = grp[grp["is_exclusive"]==1] if is_excl else grp
        tname = str(pref.iloc[0]["offer_type_name"] or "").strip()
        info[g] = {"exclusive": is_excl, "type_name": tname}
        if is_excl: excl.add(g)
    return info, excl

def offer_label(giata, offers_map):
    """Short label for the Offer column. '🌟 Exclusive' / 'Offer' / ''."""
    o = offers_map.get(norm_giata(giata))
    if not o: return ""
    return "🌟 Exclusive" if o["exclusive"] else "Offer"

def load_completions():
    """Shared action log: one row per (giata, pricing data date) that's been actioned."""
    conn = get_conn()
    try: c = pd.read_sql_query("SELECT * FROM completions", conn)
    except Exception: c = pd.DataFrame()
    conn.close()
    if not c.empty:
        c["giata"] = c["giata"].map(norm_giata)
        c["done"]  = pd.to_numeric(c["done"], errors="coerce").fillna(0).astype(int)
    return c

def save_completions(edited, file_date):
    """Upsert the edited progress rows for one pricing data date. A row with Done unchecked and
    no note/initials is removed (un-marked)."""
    conn = get_conn(); now = datetime.now().isoformat()
    for _, r in edited.iterrows():
        g = norm_giata(r.get("Giata", r.get("giata","")))
        if not g: continue
        done = 1 if r.get("Done") else 0
        note = str(r.get("Note","") or "").strip()
        who  = str(r.get("By","") or "").strip()
        if done or note or who:
            conn.execute("""INSERT OR REPLACE INTO completions(
                giata,file_date,hotel_name,destination,done,action_taken,note,who,updated_at)
                VALUES(?,?,?,?,?,?,?,?,?)""",
                (g, file_date, str(r.get("Hotel","")), str(r.get("Dest","")), done,
                 str(r.get("Sugg. Δ","")), note, who, now))
        else:
            conn.execute("DELETE FROM completions WHERE giata=? AND file_date=?", (g, file_date))
    conn.commit(); conn.close()

def build_completed_map(comp):
    """giata -> most recent pricing data date the hotel was marked done (for the table flag)."""
    if comp is None or comp.empty: return {}
    done = comp[comp["done"]==1]
    if done.empty: return {}
    return {g: sorted(grp["file_date"].astype(str))[-1] for g, grp in done.groupby("giata")}
...
def _grouped_month_fig(wr, mk, bk, bar_metric, line1, line2, series_keys, month_keys, month_labels, x_title):
    """Core builder: grouped columns + up to two lines, one coloured series per `series_keys`.
    wr/mk/bk are MultiIndex Series keyed (series_key, month_key) (bk may be None).
    Win-rate and markup values are rounded to 2dp for display. Returns (figure, line_specs)."""
    def r2(x):
        try:
            x = float(x); return round(x,2) if x==x else None
        except Exception: return None
    def series(metric, s):
        d = {"Bookings":bk, "Win Rate %":wr, "Avg Markup %":mk}[metric]
        if d is None: return [None]*len(month_keys)
        return [(d.get((s,k)) if metric=="Bookings" else r2(d.get((s,k)))) for k in month_keys]
    ...
    LINE_COLORS = ["#F39C12","#9B59B6"]   # solid, distinct line colours for the single-series view
    single = len(series_keys) == 1
    used = {bar_metric}; line_specs = []
    for lm in [line1, line2]:
        if lm and lm != "None" and lm not in used:
            used.add(lm); line_specs.append(lm)
    fig = go.Figure()
    for i,s in enumerate(series_keys):
        c = palette[i % len(palette)]
        fig.add_trace(go.Bar(x=month_labels, y=series(bar_metric,s),
                             name=(bar_metric if single else str(s)),
                             legendgroup=str(s), marker_color=c, offsetgroup=str(i)))
    for li, lm in enumerate(line_specs):
        ax = ["y2","y3"][li]
        for i,s in enumerate(series_keys):
            if single:
                lc, dsh, lname, showleg, lw, ms = LINE_COLORS[li], None, lm, True, 3, 6
            else:
                lc, dsh, lname, showleg, lw, ms = palette[i % len(palette)], ["dash","dot"][li], str(s), False, 2, 4
            fig.add_trace(go.Scatter(x=month_labels, y=series(lm,s), name=lname,
                legendgroup=(lm if single else str(s)), showlegend=showleg,
                mode="lines+markers", yaxis=ax,
                line=dict(color=lc, width=lw, dash=dsh), marker=dict(size=ms), connectgaps=False))
    ...
    n_right = len(line_specs)
    layout = dict(barmode="group", height=480, hovermode="x unified",
        xaxis=dict(title=x_title, domain=[0, 0.90] if n_right==2 else [0,1.0]),
        yaxis=dict(title=AX[bar_metric][1], range=AX[bar_metric][0]),
        legend=dict(orientation="h", yanchor="bottom", y=1.03))
    if n_right >= 1:
        layout["yaxis2"] = dict(title=AX[line_specs[0]][1], range=AX[line_specs[0]][0],
            overlaying="y", side="right", anchor="x", showgrid=False)
    if n_right == 2:
        layout["yaxis3"] = dict(title=AX[line_specs[1]][1], range=AX[line_specs[1]][0],
            overlaying="y", side="right", anchor="free", position=1.0, showgrid=False)
    fig.update_layout(**layout)
    return fig, line_specs

def dest_month_grouped(v, tx, bar_metric, line1, line2, max_dests=6, yoy=False):
    """Grouped columns + up to two lines by travel month. Coloured by destination, or — when
    `yoy=True` — by departure YEAR (month-of-year on the x-axis) for a Year-on-Year comparison.
    Returns (figure, series_shown, total_series, active_line_metrics) or None."""
    ...
    if not yoy:
        ...
        wr = g.set_index(["destination","dep_month"])["winrate"]
        mk = g.set_index(["destination","dep_month"])["markup"]
        bk = None; bk_months = set()
        if tx is not None and not tx.empty:
            ...
            if not t.empty:
                ...
                bk_months = set(i[1] for i in bk.index)
        order  = comp["destination"].value_counts().index.tolist()
        skeys  = order[:max_dests]
        months = sorted(set(g["dep_month"]) | bk_months)
        labels = [pd.to_datetime(m+"-01").strftime("%b %Y") for m in months]
        fig, ls = _grouped_month_fig(wr, mk, bk, bar_metric, line1, line2,
                                     skeys, months, labels, "Departure (Travel) Month")
        return fig, skeys, len(order), ls

    # Year-on-Year: series = departure year, x = month-of-year
    comp["yr"]  = comp["dep_month"].str[:4]
    comp["moy"] = comp["dep_month"].str[5:7]
    g = comp.groupby(["yr","moy"]).agg(
        wins=("result", lambda s:(s=="Win").sum()), n=("result","size"),
        markup=("current_margin","mean")).reset_index()
    g["winrate"] = g["wins"]/g["n"]*100
    wr = g.set_index(["yr","moy"])["winrate"]
    mk = g.set_index(["yr","moy"])["markup"]
    bk = None; bk_moys = set()
    if tx is not None and not tx.empty:
        g2d = dict(zip(v["giata"].map(norm_giata), v["destination"]))
        t = tx.copy(); t["destination"] = t["giata"].map(lambda x: g2d.get(norm_giata(x)))
        t = t[t["destination"].notna() & (t["dep_month"].astype(str).str.len()==7)]
        if not t.empty:
            t["yr"]  = t["dep_month"].str[:4]; t["moy"] = t["dep_month"].str[5:7]
            bk = t.groupby(["yr","moy"])["bookings"].sum()
            bk_moys = set(i[1] for i in bk.index)
    years  = sorted(comp["yr"].unique())
    # Order month-of-year so the axis flows chronologically from the earliest month in the
    # data (e.g. a Jun-2026→May-2027 window reads Jun…May, not Jan…Dec with 2027 on the left).
    start = int(comp["dep_month"].min()[5:7])
    moys   = sorted(set(g["moy"]) | bk_moys, key=lambda m: (int(m) - start) % 12)
    labels = [pd.to_datetime("2000-"+m+"-01").strftime("%b") for m in moys]
    fig, ls = _grouped_month_fig(wr, mk, bk, bar_metric, line1, line2,
                                 years, moys, labels, "Month of Year")
    return fig, years, len(years), ls
...
def build_rule_text_map(rules_df, is_package=True):
    """Build, once, a giata → editable rule text map so every hotel table can show the exact
    markup rule to change in-system. Value = GeneratedText of the cleanest single-hotel consumer
    rule (Exclusive first, then package/hotel level, then fewest conditions), or a '➕ add a
    single-hotel rule' marker for hotels whose markup comes only from shared/blanket rules."""
    if rules_df is None or rules_df.empty:
        return {}
    mk = rules_df[(rules_df["enabled"]==1) &
                  rules_df["verb"].isin(["MarkupNew","MarkupPerPerson"])].copy()
    ispkg = mk["ispkg"].fillna("")
    mk = mk[ispkg != ("false" if is_package else "true")]
    consumer = mk[mk["layer"].str.startswith("Consumer")].copy()
    if consumer.empty:
        return {}
    consumer["gn"] = pd.to_numeric(consumer["giata_n"], errors="coerce").fillna(0)
    single = consumer[consumer["gn"]==1].copy()
    single["excl"] = (single["layer"]=="Consumer (Exclusive)").astype(int)
    single["lvl_pref"] = single["applies_to"].map(
        lambda a: 1 if ((is_package and a=="15") or ((not is_package) and a=="12")) else 0)
    single = single.sort_values(["excl","lvl_pref","n_conds"], ascending=[False,False,True])
    ...
    for _, r in single.iterrows():
        g = norm_giata(r["giatas"])
        if g and g not in out:
            out[g] = r["generated_text"]
    # giatas covered only by shared/blanket consumer rules → flag to create a single-hotel rule
    for gl in consumer["giatas"]:
        if not gl: continue
        for g in gl.split(","):
            g = norm_giata(g)
            if g and g not in out:
                out[g] = "➕ add a single-hotel rule (currently shared/blanket only)"
    return out
...
with st.sidebar:
    ...
    st.markdown("### 🌟 Upload Hotel Offers")
    ofile = st.file_uploader("Active offers export (Giata, Booking/Travel dates, Is Exclusive)",
        type=["csv","xlsx"], key="ou")
    if ofile:
        try:
            odf = pd.read_csv(ofile) if ofile.name.endswith(".csv") else pd.read_excel(ofile)
            om = {c.lower().strip(): c for c in odf.columns}
            gcol = next((om[k] for k in om if k=="giata"), None)
            if gcol is None:
                st.error("Need a 'Giata' column in the offers file.")
            else:
                def pick(*names):
                    return next((om[k] for k in om if k in names), None)
                col = dict(hotel=pick("hotel name","hotel_name","hotel"),
                           dest=pick("hermes destination","destination"),
                           tname=pick("offer type name","offer_type_name","offer type"),
                           excl=pick("is exclusive","is_exclusive","exclusive"),
                           status=pick("deal status","deal_status","status"),
                           bf=pick("booking from","booking_from","book from"),
                           bt=pick("booking to","booking_to","book to"),
                           tf=pick("travel from","travel_from"),
                           tt=pick("travel to","travel_to"),
                           det=pick("tactical offer details","details","offer details"))
                def colvals(key):
                    c = col[key]
                    return odf[c] if c is not None else pd.Series([None]*len(odf))
                def dt(s): return pd.to_datetime(s, errors="coerce")
                excl_raw = colvals("excl")
                out = pd.DataFrame({
                    "giata": odf[gcol].map(norm_giata),
                    "hotel_name": colvals("hotel").astype(str),
                    "destination": colvals("dest").astype(str),
                    "offer_type_name": colvals("tname").astype(str),
                    "is_exclusive": excl_raw.map(lambda x: 1 if str(x).strip().lower() in ("true","1","yes","y") else 0),
                    "deal_status": colvals("status").astype(str),
                    "book_from": dt(colvals("bf")).astype(str),
                    "book_to":   dt(colvals("bt")).astype(str),
                    "travel_from": dt(colvals("tf")).astype(str),
                    "travel_to":   dt(colvals("tt")).astype(str),
                    "details": colvals("det").astype(str).str.slice(0,500),
                })
                out = out[out["giata"].str.len()>0]
                out["uploaded_at"] = datetime.now().isoformat()
                conn = get_conn()
                out.to_sql("offers", conn, if_exists="replace", index=False)
                conn.commit(); conn.close()
                _, live_excl = load_offers_map()
                st.success(f"✅ {len(out):,} offers · {out['giata'].nunique():,} hotels · "
                           f"{int(out['is_exclusive'].sum()):,} exclusive ({len(live_excl):,} live exclusive today)")
        except Exception as e: st.error(f"Offers error: {e}")
...
# A giata can appear under different hotel names across uploads (e.g. a hotel was renamed).
# Giata is the key — collapse every giata to ONE name: the one from its most recent sheet.
if not df.empty and {"giata","hotel_name","file_date"}.issubset(df.columns):
    latest_name = (df.sort_values("file_date").groupby("giata")["hotel_name"].last())
    df["hotel_name"] = df["giata"].map(latest_name).fillna(df["hotel_name"])
bk = load_bookings()
...
rule_text_map = build_rule_text_map(rules_df, is_package=True)
offers_map, excl_giatas = load_offers_map()
HAS_OFFERS = bool(offers_map)
completions_df = load_completions()
completed_map = build_completed_map(completions_df)
LATEST_DATE = (sorted(df["file_date"].dropna().astype(str).unique())[-1]
               if not df.empty and df["file_date"].notna().any() else "")
if not df.empty:
...
# TABS
# ══════════════════════════════════════════════════════════════════════════════
tabs = st.tabs(["⚡ Master Queue","Overview","🏆 Top Sellers","Hotel Actions",
                ...
                "✅ Progress","🏢 Suppliers","📖 How It Works"])
...
def with_offer(show_df):
    """Append an 'Offer' column flagging a live offer (🌟 Exclusive / Offer) per hotel.
    No-op when no offers file is loaded."""
    if HAS_OFFERS and "Giata" in show_df.columns and "Offer" not in show_df.columns:
        show_df["Offer"] = show_df["Giata"].map(lambda g: offer_label(g, offers_map))
    return show_df

def with_rule(show_df):
    """Append a 'Rule (GeneratedText)' column showing the exact markup rule to change in-system
    for that hotel (the editable single-hotel consumer rule, or a marker to add one). No-op when
    no rules file is loaded, or when the table already carries a rule column (Markup Levers)."""
    if (HAS_RULES and "Giata" in show_df.columns
            and "Rule (GeneratedText)" not in show_df.columns and "Rule" not in show_df.columns):
        show_df["Rule (GeneratedText)"] = show_df["Giata"].map(
            lambda g: rule_text_map.get(norm_giata(g), ""))
    return show_df

def with_completed(show_df):
    """Append a '✅ Done' column showing the most recent pricing data date the hotel was
    actioned (shared across users). Blank if not yet done. No-op without a Giata column."""
    if completed_map and "Giata" in show_df.columns and "✅ Done" not in show_df.columns:
        show_df["✅ Done"] = show_df["Giata"].map(
            lambda g: f"✅ {completed_map[norm_giata(g)]}" if norm_giata(g) in completed_map else "")
    return show_df

def colour_key_html():
    """Render the shared colour-key legend (used on Overview and Top Sellers)."""
    st.markdown(
        "<div style='display:flex;gap:14px;flex-wrap:wrap;align-items:center;font-size:0.82rem;margin-bottom:6px'>"
        "<b>Colour key:</b>"
        "<span style='background:rgba(39,124,78,0.40);padding:2px 10px;border-radius:6px'>Win / raise margin ↑</span>"
        "<span style='background:rgba(192,57,43,0.40);padding:2px 10px;border-radius:6px'>Lose / cut margin ↓</span>"
        "<span style='background:rgba(212,175,55,0.30);padding:2px 10px;border-radius:6px'>Hold / no comp</span>"
        "<span style='background:rgba(27,111,212,0.30);padding:2px 10px;border-radius:6px'>Product / cost fix</span>"
        "<span style='background:rgba(155,89,182,0.45);padding:2px 10px;border-radius:6px'>🌟 Exclusive offer</span>"
        "</div>", unsafe_allow_html=True)

def inline_progress(scope_df, key):
    """Reusable inline progress logger placed under a hotel table. Lets the team tick ✅ Done /
    add initials + a note for the hotels shown — against the current pricing data date — and save
    to the shared log without leaving the tab. Kept in an expander so the colour-coded table above
    stays as the main view."""
    if df.empty or scope_df is None or getattr(scope_df, "empty", True): return
    if "Giata" not in scope_df.columns: return
    hcol = next((c for c in ["Hotel","hotel_name","Hotel Name"] if c in scope_df.columns), None)
    dcol = next((c for c in ["Dest","destination","Destination"] if c in scope_df.columns), None)
    acol = next((c for c in ["Sugg. Markup Δ","Sugg. Δ","Action","Suggested"] if c in scope_df.columns), None)
    work_date = st.session_state.get("work_date", LATEST_DATE)
    with st.expander(f"✅ Log progress for these hotels — against data date {work_date}"):
        base = scope_df.drop_duplicates("Giata")[[c for c in ["Giata",hcol,dcol,acol] if c]].copy()
        ren = {}
        if hcol: ren[hcol]="Hotel"
        if dcol: ren[dcol]="Dest"
        if acol: ren[acol]="Sugg. Δ"
        base = base.rename(columns=ren)
        for needed in ["Hotel","Dest","Sugg. Δ"]:
            if needed not in base.columns: base[needed] = ""
        comp = load_completions()
        cur = (comp[comp["file_date"].astype(str)==str(work_date)].set_index("giata")
               if not comp.empty else pd.DataFrame())
        def cv(g,col,d):
            gg=norm_giata(g)
            return (cur.loc[gg,col] if (not cur.empty and gg in cur.index) else d)
        base["Done"] = base["Giata"].map(lambda g: bool(cv(g,"done",0)))
        base["By"]   = base["Giata"].map(lambda g: str(cv(g,"who","") or ""))
        base["Note"] = base["Giata"].map(lambda g: str(cv(g,"note","") or ""))
        base = base[["Done","Giata","Hotel","Dest","Sugg. Δ","By","Note"]].reset_index(drop=True)
        # stable-but-scoped key so edits persist while editing, reset when scope/date changes
        scope_sig = str(abs(hash((str(work_date), tuple(base["Giata"])))) % (10**8))
        edited = st.data_editor(
            base, hide_index=True, use_container_width=True, key=f"ip_{key}_{scope_sig}",
            column_config={
                ...
                "Sugg. Δ": st.column_config.TextColumn("Sugg. Δ", disabled=True),
                "By": st.column_config.TextColumn("By (initials)"),
                "Note": st.column_config.TextColumn("Note (what was changed)"),
            })
        if st.button("💾 Save progress", key=f"ipsave_{key}_{scope_sig}", type="primary"):
            save_completions(edited, work_date)
            st.success("Saved — the ✅ Done column updates across all tabs.")
            st.rerun()

# ── Table colour highlighting ───────────────────────────────────────────────────
_GREEN = "background-color: rgba(39,124,78,0.40)"
_RED   = "background-color: rgba(192,57,43,0.40)"
_AMBER = "background-color: rgba(212,175,55,0.30)"
_BLUE  = "background-color: rgba(27,111,212,0.30)"
_PURP  = "background-color: rgba(155,89,182,0.45); font-weight:bold"

def _c_action(val):
    s = str(val).lower()
    if any(k in s for k in ["raise","headroom","capture","advantage","win /"]): return _GREEN
    if any(k in s for k in ["cut","reduce","flip","lose /"]):                   return _RED
    if any(k in s for k in ["product","cost","visibility","gap","fix"]):        return _BLUE
    if any(k in s for k in ["hold","maintain","no direct comp","no comp","unsure","monitor","destination-guided","keep"]): return _AMBER
    return ""
def _c_change(val):
    s = str(val).strip()
    if s.startswith("+") and not s.startswith("+0.0"): return _GREEN
    if s.startswith("-") and not s.startswith("-0.0"): return _RED
    return ""
def _c_pct(val):
    try: x = float(str(val).replace("%","").strip())
    except Exception: return _AMBER if str(val).strip().lower() in ("none","—","") else ""
    return _GREEN if x >= 70 else (_AMBER if x >= 50 else _RED)
def _c_offer(val):
    s = str(val).lower()
    if "exclusive" in s: return _PURP
    if "offer" in s:     return _BLUE
    return ""
def _c_result(val):
    s = str(val).lower()
    if s == "win":  return _GREEN
    if s == "lose": return _RED
    if "no comp" in s: return _AMBER
    return ""
def _c_verdict(val):
    return _RED if "product gap" in str(val).lower() else (_GREEN if "reachable" in str(val).lower() else "")

def style_table(df):
    """Colour-code action/result/offer/win-rate/change columns for at-a-glance reading.
    Returns a pandas Styler (renders in st.dataframe). Safe on any frame."""
    ...
    def sub(cols, fn):
        ...
        if cc:
            try: return sty.map(fn, subset=cc)
            except Exception: return sty
        return sty
    sty = sub(["Action","Note","Stance","Dest Stance","Recommendation"], _c_action)
    sty = sub(["Result"], _c_result)
    sty = sub(["Verdict"], _c_verdict)
    sty = sub(["Offer"], _c_offer)
    sty = sub(["Sugg. Markup Δ","Change","Sugg Δ","Markup Δ"], _c_change)
    sty = sub(["WR Now","WR Est","Win Rate","Win Rate %","Proj. Win Rate","Proj Win Rate"], _c_pct)
    sty = sub(["Wins"], lambda v: _GREEN)
    sty = sub(["Loses","Losses"], lambda v: _RED)
    sty = sub(["✅ Done"], lambda v: _GREEN if str(v).strip() else "")
    return sty

def styled_hotel(df, fmt=None):
    """One-stop for per-hotel tables: add booking + offer columns, colour-highlight the
    action/result/offer/win-rate columns, and apply optional number formatting."""
    df = with_completed(with_rule(with_offer(with_bkg(df))))
    sty = style_table(df)
    if fmt and hasattr(sty, "format"):
        try: sty = sty.format(fmt)
        except Exception: pass
    return sty
...
with tabs[0]:
    ...
    if acts:
        ...
        st.dataframe(styled_hotel(show, {"D2 £pp":"{:.0f}","Comp £pp":"{:.0f}","Diff %":"{:.1f}%"}),
                     use_container_width=True, height=550)
        inline_progress(show, "mq")
    else:
...
with tabs[1]:
    ...
    with col1:
        ...
        st.plotly_chart(fig,use_container_width=True, key="ovw_results_window")
    with col2:
        if not vc.empty:
            ...
            st.plotly_chart(fig2,use_container_width=True, key="ovw_winrate_dest")
    ...
    if HAS_BKG:
        ...
        if db:
            ...
            if not bdest.empty:
                ...
                st.plotly_chart(figb,use_container_width=True, key="ovw_bookings_dest")

    # By travel month — one destination at a time, columns + up to two solid coloured lines
    comp_any = v[v["result"].isin(["Win","Lose"])]
    if not comp_any.empty and comp_any["dep_month"].astype(str).str.len().eq(7).any():
        st.markdown("#### 📈 By Travel Month")
        st.caption("Pick one destination for a clear read of win rate, bookings and markup by "
                   "travel (departure) month. Narrow further by window, comparison run date and "
                   "booking dates.")
        fc1, fc2, fc3 = st.columns(3)
        d_opts = v["destination"].value_counts().index.tolist()   # busiest first
        f_dest = fc1.selectbox("Destination", d_opts, index=0, key="fv_d")
        w_opts = sorted(v["dep_window"].dropna().unique().tolist())
        f_wins = fc2.multiselect("Departure window(s)", w_opts, default=w_opts, key="fv_w")
        r_opts = sorted(v["file_date"].dropna().unique().tolist())
        f_runs = fc3.multiselect("Comparison run date(s)", r_opts, default=r_opts, key="fv_r",
                                 help="Pick which comparison upload(s) to view.")
        txf = bk_tx.copy() if HAS_TX else None
        if HAS_TX and not bk_tx.empty:
            bc1, bc2 = st.columns(2)
            bm = sorted([m for m in bk_tx["booked_month"].dropna().unique() if len(str(m))==7])
            dm = sorted([m for m in bk_tx["dep_month"].dropna().unique()    if len(str(m))==7])
            if len(bm) > 1:
                b0,b1 = bc1.select_slider("Booked-month range", options=bm,
                                          value=(bm[0],bm[-1]), key="fv_bm")
                txf = txf[(txf["booked_month"]>=b0) & (txf["booked_month"]<=b1)]
            if len(dm) > 1:
                d0,d1 = bc2.select_slider("Departure-month range", options=dm,
                                          value=(dm[0],dm[-1]), key="fv_dm")
                txf = txf[(txf["dep_month"]>=d0) & (txf["dep_month"]<=d1)]
        mc1, mc2, mc3, mc4 = st.columns(4)
        metrics2 = (["Bookings"] if HAS_TX else []) + ["Win Rate %","Avg Markup %"]
        lopts2 = ["None"] + metrics2
        bar2 = mc1.radio("Columns", metrics2, index=metrics2.index("Win Rate %"),
                         horizontal=True, key="fv_bar")
        l1d = ("Bookings" if (HAS_TX and bar2 != "Bookings")
               else ("Avg Markup %" if bar2 != "Avg Markup %" else "None"))
        line1b = mc2.radio("Line 1", lopts2, index=lopts2.index(l1d), horizontal=True, key="fv_l1")
        l2d = "Avg Markup %" if "Avg Markup %" not in (bar2, line1b) else "None"
        line2b = mc3.radio("Line 2", lopts2, index=lopts2.index(l2d), horizontal=True, key="fv_l2")
        yoy = mc4.checkbox("Year-on-Year", key="fv_yoy",
                           help="Compare departure years month-by-month (needs >1 year of data).")
        vf = v[(v["destination"]==f_dest) & v["dep_window"].isin(f_wins) & v["file_date"].isin(f_runs)]
        if vf.empty:
            st.info("No data for the chosen filters.")
        else:
            res2 = dest_month_grouped(vf, txf if HAS_TX else None, bar2, line1b, line2b, yoy=yoy)
            if res2:
                fig2, skeys, stot, ls2 = res2
                if yoy and len(skeys) < 2:
                    st.caption("ℹ️ Year-on-Year needs departures spanning more than one year. Showing "
                               "what's available now — this becomes a true YoY comparison once multiple "
                               "years of comparison data are loaded into the database.")
                if len(skeys) == 1:
                    cap2 = f"**{f_dest}** by travel month" + (" (Year-on-Year)" if yoy else "") + f". Bars = **{bar2}**"
                    if ls2: cap2 += "; solid lines = **" + "**, **".join(ls2) + "** (see legend)"
                else:
                    cap2 = f"**{f_dest}**, coloured by departure year. Bars = **{bar2}**"
                    if len(ls2) >= 1: cap2 += f"; dashed = **{ls2[0]}**"
                    if len(ls2) == 2: cap2 += f"; dotted = **{ls2[1]}**"
                cap2 += ". Win-rate and markup shown to 2dp."
                st.caption(cap2)
                st.plotly_chart(fig2, use_container_width=True, key="ovw_dest_month_focused")
...
with tabs[2]:
    ...
    st.caption("**Sugg. Markup Δ** = high-level change to Total Markup (in pp) per hotel, "
               ...
               + ("  Hotels with a **live exclusive offer** are always included." if HAS_OFFERS else ""))
    ts_all = v.groupby("giata").agg(
        hotel_name=("hotel_name","first"),
        Dest=("destination","first"),Comp=("competitor","first"),
        Rows=("result","count"),AvgMargin=("current_margin","mean"),
    ...
    ts_all = ts_all.sort_values("Bookings" if HAS_BKG else "Rows", ascending=False)
    ts = ts_all.head(15)
    # Additionally fold in any hotel with a LIVE EXCLUSIVE offer (no duplicates)
    if HAS_OFFERS and excl_giatas:
        extra = ts_all[ts_all["giata"].map(norm_giata).isin(excl_giatas)
                       & ~ts_all["giata"].isin(ts["giata"])]
        if not extra.empty:
            ts = pd.concat([ts, extra], ignore_index=True)
    ts = ts.reset_index(drop=True)

    # Suggested markup change per hotel — keyed on GIATA only (all its rows, regardless of name)
    sug = [suggest_markup_change(v[v["giata"]==g]) for g in ts["giata"]]
    ...
    if HAS_BKG:
        ...
        ts.columns = ["Giata","Hotel","Dest","Comp","Rows","Avg Margin","Loss?",
                      "Sugg. Markup Δ","Proj. Win Rate","Proj. Markup","Note"]
    ts = with_completed(with_rule(with_offer(ts)))
    st.dataframe(style_table(ts),use_container_width=True)
    inline_progress(ts, "topsellers")
...
with tabs[3]:
    ...
    st.dataframe(styled_hotel(show3, {"D2 £pp":"{:.0f}","Comp £pp":"{:.0f}","Diff %":"{:.1f}%"}),
                 use_container_width=True,height=600)
    inline_progress(show3, "hotelactions")
...
with tabs[4]:
    ...
    st.dataframe(styled_hotel(show4, {"D2 £pp":"{:.0f}","Comp £pp":"{:.0f}","Diff %":"{:.1f}%"}),
                 use_container_width=True,height=550)
    inline_progress(show4, "reduce")
...
with tabs[5]:
    ...
    st.dataframe(styled_hotel(show5, {"D2 £pp":"{:.0f}","Comp £pp":"{:.0f}","Win %":"{:.1f}%"}),
                 use_container_width=True,height=500)
    inline_progress(show5, "raise")
...
with tabs[6]:
    ...
    st.dataframe(styled_hotel(show6, {"D2 £pp":"{:.0f}","Comp £pp":"{:.0f}",
                                      "Diff %":"{:.1f}%","Margin %":"{:.1f}%"}),
                 use_container_width=True,height=500)
    inline_progress(show6, "maintain")
...
with tabs[7]:
    ...
    if losses.empty:
        ...
        st.dataframe(styled_hotel(out), use_container_width=True, height=460)
        inline_progress(out, "productgaps")
...
with tabs[8]:
    ...
    if nc2.empty:
        ...
        st.dataframe(styled_hotel(show8),use_container_width=True,height=500)
        inline_progress(show8, "nocomp")
...
with tabs[10]:
    ...
    if cl2:
        ...
        st.dataframe(styled_hotel(pd.DataFrame(cl2)),use_container_width=True)
    if bw2:
        st.success(f"🔵 {len(bw2)} big win opportunities")
        st.dataframe(styled_hotel(pd.DataFrame(bw2)),use_container_width=True)
...
with tabs[12]:
    ...
    st.dataframe(styled_hotel(adv2),use_container_width=True,height=500)
...
with tabs[13]:
    ...
    if not v_loses.empty:
        ...
        st.dataframe(styled_hotel(tl, {"D2 £pp":"{:.0f}","Comp £pp":"{:.0f}",
    ...
    if not v_wins.empty:
        ...
        st.dataframe(styled_hotel(bw3, {"Win %":"{:.1f}%","Margin":"{:.1f}%"}),use_container_width=True)
...
with tabs[14]:
    ...
    if not v.empty:
        ...
        if sc.empty:
            ...
            st.plotly_chart(fig_sc,use_container_width=True, key="trd_price")

    # Win rate & avg markup by departure month (aggregate across the selection)
    if not v.empty:
        dest_lbl_w = ", ".join(v["destination"].unique().tolist())
        wm_src = v[v["result"].isin(["Win","Lose"])].copy()
        wm_src["current_margin"] = pd.to_numeric(wm_src["current_margin"], errors="coerce")
        wm_src = wm_src[wm_src["dep_month"].astype(str).str.len()==7]
        if not wm_src.empty:
            wm = (wm_src.groupby("dep_month")
                    .agg(wins=("result", lambda s:(s=="Win").sum()), n=("result","size"),
                         Markup=("current_margin","mean")).reset_index().sort_values("dep_month"))
            wm["WinRate"] = (wm["wins"]/wm["n"]*100).round(2)
            wm["Markup"]  = wm["Markup"].round(2)
            wm["Label"]   = wm["dep_month"].apply(lambda m: pd.to_datetime(m+"-01").strftime("%b %Y"))
            figwm = go.Figure()
            figwm.add_trace(go.Bar(x=wm["Label"], y=wm["WinRate"], name="Win Rate %",
                marker_color="rgba(27,111,212,0.55)"))
            figwm.add_trace(go.Scatter(x=wm["Label"], y=wm["Markup"], name="Avg Markup %",
                mode="lines+markers", line=dict(color="#D4AF37", width=3), marker=dict(size=6), yaxis="y2"))
            figwm.add_hline(y=50, line_dash="dot", line_color="#E04A3F",
                            annotation_text="50%", annotation_position="right")
            figwm.update_layout(title=f"Win Rate &amp; Avg Markup by Departure Month — {dest_lbl_w}",
                yaxis=dict(title="Win Rate %", range=[0,100]),
                yaxis2=dict(title="Avg Markup %", overlaying="y", side="right", showgrid=False),
                height=340, hovermode="x unified", barmode="group",
                legend=dict(orientation="h", yanchor="bottom", y=1.02))
            st.plotly_chart(figwm, use_container_width=True, key="trd_winrate_markup_month")
    ...
    if not v.empty:
        ...
        if not mk_m.empty:
            ...
            figm.update_layout(title=f"Avg Markup &amp; Bookings by Departure Month — {dest_lbl_m}",
                ...
                legend=dict(orientation="h", yanchor="bottom", y=1.02))
            st.plotly_chart(figm, use_container_width=True, key="trd_markup_bkg")
    ...
    with col3:
        ...
        if not wr_src.empty and wr_src["dep_month"].ne("").any():
            ...
            fig_wr.update_layout(title=f"Win Rate &amp; Bookings by Departure Month — {dest_lbl2}",
                ...
                legend=dict(orientation="h",yanchor="bottom",y=1.02))
            st.plotly_chart(fig_wr,use_container_width=True, key="trd_winrate_month")
    ...
    with col4:
        ...
        if not wr_run_src.empty and wr_run_src["file_date"].nunique()>1:
            ...
            st.plotly_chart(fig_run,use_container_width=True, key="trd_winrate_run")
    ...
    if HAS_TX and not v.empty:
        ...
        if not bk_booked.empty:
            ...
            figp.update_layout(title="Booking Pace — by Booked Month (when the bookings were made)",
                ...
                legend=dict(orientation="h", yanchor="bottom", y=1.02))
            st.plotly_chart(figp, use_container_width=True, key="trd_booking_pace")
    ...
    if v["file_date"].nunique()>1:
        ...
        st.plotly_chart(fig_dr,use_container_width=True, key="trd_price_drift")
...
# ── 17. PROGRESS / ACTION LOG ────────────────────────────────────────────────────
with tabs[17]:
    st.subheader("✅ Progress / Action Log")
    st.caption("Mark hotels as actioned so the team doesn't duplicate work — tracked against the "
               "pricing data date, and shared across everyone viewing the app. Stored in the app "
               "database; it persists across sessions but is reset if the app is redeployed/restarted, "
               "so export the log periodically once you're building history.")
    if df.empty:
        st.info("Upload pricing data first.")
    else:
        dates = sorted(df["file_date"].dropna().astype(str).unique().tolist(), reverse=True)
        dflt = st.session_state.get("work_date", dates[0]) if dates else None
        work_date = st.selectbox("Pricing data date being actioned", dates,
                                 index=(dates.index(dflt) if dflt in dates else 0), key="prog_date")
        st.session_state["work_date"] = work_date
        v = tab_filters(df, "prog")
        dsel = sorted(v["destination"].dropna().unique().tolist())
        wsel = sorted(v["dep_window"].dropna().unique().tolist())
        sig = f"{work_date}|{','.join(dsel)}|{','.join(wsel)}"
        comp = load_completions()
        cur = comp[comp["file_date"].astype(str)==work_date].set_index("giata") if not comp.empty else pd.DataFrame()

        if st.session_state.get("prog_sig") != sig:
            hotels = v.drop_duplicates("giata")[["giata","hotel_name","destination"]].copy()
            sug = [suggest_markup_change(v[v["giata"]==g]) for g in hotels["giata"]]
            hotels["Sugg. Δ"] = [f"{s['delta']:+.1f}pp" if abs(s['delta'])>=0.05 else "0.0pp" for s in sug]
            if HAS_BKG:
                hotels["Bkgs"] = hotels["giata"].map(lambda g: bk_map.get(norm_giata(g),0))
                hotels = hotels.sort_values("Bkgs", ascending=False)
            hotels = hotels.rename(columns={"giata":"Giata","hotel_name":"Hotel","destination":"Dest"})
            def cv(g, col, default): 
                gg=norm_giata(g); 
                return (cur.loc[gg,col] if (not cur.empty and gg in cur.index) else default)
            hotels["Done"] = hotels["Giata"].map(lambda g: bool(cv(g,"done",0)))
            hotels["By"]   = hotels["Giata"].map(lambda g: str(cv(g,"who","") or ""))
            hotels["Note"] = hotels["Giata"].map(lambda g: str(cv(g,"note","") or ""))
            st.session_state["prog_base"] = hotels.reset_index(drop=True)
            st.session_state["prog_sig"]  = sig
            st.session_state.pop("prog_editor", None)
        base = st.session_state["prog_base"]

        cols = ["Done","Giata","Hotel","Dest"] + (["Bkgs"] if HAS_BKG else []) + ["Sugg. Δ","By","Note"]
        edited = st.data_editor(
            base[cols], hide_index=True, use_container_width=True, height=460, key="prog_editor",
            column_config={
                "Done": st.column_config.CheckboxColumn("✅ Done"),
                "Giata": st.column_config.TextColumn(disabled=True),
                "Hotel": st.column_config.TextColumn(disabled=True),
                "Dest": st.column_config.TextColumn(disabled=True),
                "Bkgs": st.column_config.NumberColumn("📊 Bkgs", disabled=True),
                "Sugg. Δ": st.column_config.TextColumn("Sugg. Δ", disabled=True),
                "By": st.column_config.TextColumn("By (initials)"),
                "Note": st.column_config.TextColumn("Note (what was changed)"),
            })
        c1, c2 = st.columns([1,3])
        if c1.button("💾 Save progress", type="primary"):
            save_completions(edited, work_date)
            st.session_state.pop("prog_sig", None)     # force refresh from DB
            st.session_state.pop("prog_editor", None)
            st.success("Saved."); st.rerun()

        st.markdown("#### 📋 Action log (all dates)")
        if comp.empty:
            st.caption("Nothing logged yet. Tick **Done**, add initials/notes, and Save.")
        else:
            log = comp.copy()
            log["done"] = log["done"].map({1:"✅",0:""})
            log = log.sort_values("updated_at", ascending=False)[
                ["done","file_date","giata","hotel_name","destination","action_taken","note","who","updated_at"]]
            log.columns = ["Done","Data Date","Giata","Hotel","Dest","Action Taken","Note","By","Logged At"]
            st.dataframe(log, use_container_width=True, height=320)
            st.download_button("⬇ Export action log (CSV)", log.to_csv(index=False).encode(),
                               "action_log.csv", "text/csv")
            with st.expander("⚠ Clear the entire action log"):
                st.caption("Removes all completion records for everyone. This cannot be undone.")
                if st.button("Clear action log"):
                    conn = get_conn(); conn.execute("DELETE FROM completions"); conn.commit(); conn.close()
                    st.session_state.pop("prog_sig", None); st.rerun()

# ── 18. SUPPLIERS ───────────────────────────────────────────────────────────────
with tabs[18]:
    st.subheader("🏢 Supplier Performance")
    st.caption("Supplier codes after the dash are merged to the parent supplier "
               "(e.g. OTS - AMXCUN75FI → OTS). Named sub-brands are kept (e.g. Juniper - Travco).")
    v = tab_filters(df, "sup")
    if "supplier" not in v.columns or v["supplier"].fillna("").eq("").all():
        st.info("No supplier data found. The Supplier name is read from column I of the D2 pricing "
                "files — re-upload the pricing files to populate this tab.")
    else:
        sv = v[v["supplier"].fillna("").ne("")].copy()
        comp = sv[sv["result"].isin(["Win","Lose"])]
        g = sv.groupby("supplier").agg(
            Rows=("supplier","size"),
            Hotels=("giata", lambda s: s.nunique()),
        ).reset_index()
        wl = comp.groupby("supplier").agg(
            Wins=("result", lambda s:(s=="Win").sum()),
            Loses=("result", lambda s:(s=="Lose").sum()),
            AvgMargin=("current_margin","mean")).reset_index()
        g = g.merge(wl, on="supplier", how="left")
        # bookings per supplier = sum of its distinct hotels' bookings
        if HAS_BKG:
            gh = sv.drop_duplicates(["supplier","giata"])[["supplier","giata"]].copy()
            gh["b"] = gh["giata"].map(lambda x: bk_map.get(norm_giata(x), 0))
            bks = gh.groupby("supplier")["b"].sum()
            g["Bookings"] = g["supplier"].map(bks).fillna(0)
        g["Wins"]  = g["Wins"].fillna(0).astype(int)
        g["Loses"] = g["Loses"].fillna(0).astype(int)
        wlsum = g["Wins"] + g["Loses"]
        g["WinRate"] = (g["Wins"] / wlsum * 100).where(wlsum > 0)
        g = g.sort_values("Rows", ascending=False)

        out = pd.DataFrame({"Supplier": g["supplier"], "Rows": g["Rows"], "Hotels": g["Hotels"]})
        if HAS_BKG: out["📊 Bookings"] = g["Bookings"].map("{:,.0f}".format)
        out["Wins"]      = g["Wins"]
        out["Loses"]     = g["Loses"]
        out["Avg Margin"]= g["AvgMargin"].map(lambda x: f"{x:.1f}%" if pd.notna(x) else "—")
        out["Win Rate"]  = g["WinRate"].map(lambda x: f"{x:.0f}%" if pd.notna(x) else "—")
        st.dataframe(style_table(out), use_container_width=True, height=560)
        st.caption(f"{len(out)} suppliers across the current selection. Wins/Loses use real "
                   f"comparisons only (no-comp excluded). Win Rate = Wins ÷ (Wins + Loses).")

# ── 19. HOW IT WORKS (reference guide for new users) ────────────────────────────
...
""")
    ...
    st.markdown("""
The app combines up to four uploads. It works with just the first, and gets smarter as you add the others.
...
| **Hotel offers export** | Active offers with Giata, booking/travel dates and an *Is Exclusive* flag. | Flags hotels with a live offer (🌟 Exclusive / Offer) in the tables, and always includes live exclusives in Top Sellers. |
...
| **Overview** | Headline picture: win rate, losses, margin, bookings, and the destination/month charts. |
| **🏆 Top Sellers** | Your biggest hotels by bookings (plus any live exclusive offer) with each hotel's suggested margin change, rule to edit, and a progress logger. |
