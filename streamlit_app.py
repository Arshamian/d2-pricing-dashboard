...
def _grouped_month_fig(wr, mk, bk, bar_metric, line1, line2, series_keys, month_keys, month_labels, x_title):
    ...
    for i,s in enumerate(series_keys):
        ...
        fig.add_trace(go.Bar(x=month_labels, y=series(bar_metric,s), name=str(s),
                             legendgroup=str(s), marker_color=c, offsetgroup=str(i)))
    for li, lm in enumerate(line_specs):
        ax = ["y2","y3"][li]; dsh = ["dash","dot"][li]
        for i,s in enumerate(series_keys):
            c = palette[i % len(palette)]
            fig.add_trace(go.Scatter(x=month_labels, y=series(lm,s), name=str(s), legendgroup=str(s),
                showlegend=False, mode="lines+markers", yaxis=ax,
                line=dict(color=c, width=2, dash=dsh), marker=dict(size=4), connectgaps=False))
...
with tabs[1]:
    ...
    # By travel month & destination — columns per destination, bars + up to two lines
    ...
    if not comp_any.empty and comp_any["dep_month"].astype(str).str.len().eq(7).any():
        ...
        metrics   = (["Bookings"] if HAS_TX else []) + ["Win Rate %","Avg Markup %"]
        line_opts = ["None"] + metrics
        cc1, cc2, cc3 = st.columns(3)
        bar_metric = cc1.radio("Columns show", metrics, index=metrics.index("Win Rate %"),
                               horizontal=True, key="dmg_bar")
        l1_default = ("Bookings" if (HAS_TX and bar_metric != "Bookings")
                      else ("Avg Markup %" if bar_metric != "Avg Markup %" else "None"))
        line1 = cc2.radio("Overlay line 1 (dashed)", line_opts,
                          index=line_opts.index(l1_default), horizontal=True, key="dmg_l1")
        l2_default = "Avg Markup %" if "Avg Markup %" not in (bar_metric, line1) else "None"
        line2 = cc3.radio("Overlay line 2 (dotted)", line_opts,
                          index=line_opts.index(l2_default), horizontal=True, key="dmg_l2")
        res = dest_month_grouped(v, bk_tx if HAS_TX else None, bar_metric, line1, line2)
        if res:
            figdm, dshown, ntot, lspecs = res
            cap = f"Each coloured column is a destination, grouped by travel (departure) month. Bars = **{bar_metric}**"
            if len(lspecs) >= 1: cap += f"; dashed line = **{lspecs[0]}**"
            if len(lspecs) == 2: cap += f"; dotted line = **{lspecs[1]}**"
            cap += ". Each metric uses its own axis."
            if ntot > len(dshown):
                cap += f"  Showing the top {len(dshown)} of {ntot} destinations by data volume."
            st.caption(cap)
            st.plotly_chart(figdm, use_container_width=True, key="ovw_dest_month_main")

        # Focused view — narrow scope by filters; optional Year-on-Year
        with st.expander("🔬 Focused view — filter by destination, window, comparison run date & booking dates"):
            fc1, fc2, fc3 = st.columns(3)
            d_opts = sorted(v["destination"].dropna().unique().tolist())
            f_dests = fc1.multiselect("Destination(s)", d_opts, default=d_opts, key="fv_d")
            ...
            f_runs = fc3.multiselect("Comparison run date(s)", r_opts, default=r_opts, key="fv_r",
                                     help="The 4th dimension — pick which comparison upload(s) to view.")
            ...
            vf = v[v["destination"].isin(f_dests) & v["dep_window"].isin(f_wins)
                   & v["file_date"].isin(f_runs)]
            if vf.empty or not f_dests:
                ...
                if res2:
                    ...
                    by = "departure year" if yoy else "destination"
                    cap2 = f"Filtered view, coloured by {by}. Bars = **{bar2}**"
                    if len(ls2) >= 1: cap2 += f"; dashed line = **{ls2[0]}**"
                    if len(ls2) == 2: cap2 += f"; dotted line = **{ls2[1]}**"
...
with tabs[2]:
    ...
    ts_all = v.groupby(["giata","hotel_name"]).agg(
