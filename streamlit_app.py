...
def dest_month_grouped(v, tx, bar_metric, line1, line2=None, max_dests=6):
    """Grouped columns by destination for `bar_metric`, with up to TWO overlaid lines
    (`line1`, `line2`) — each metric on its own y-axis so all three scales stay readable.
    Lines are per-destination (same colours); line1 is dashed, line2 is dotted.
    Returns (figure, destinations_shown, total_destinations, active_line_metrics)."""
    ...
    def series(metric, d):
        ...
        return [(bk.get((d,m)) if bk is not None else None) if metric=="Bookings"
                else base[col].get((d,m)) for m in months]

    AX = {"Bookings":(None,"Bookings"), "Win Rate %":([0,100],"Win Rate %"),
          "Avg Markup %":(None,"Markup %")}
    # active, distinct line metrics
    ...
    for li, lm in enumerate(line_specs):
        ...
        for i,d in enumerate(dests):
            c = palette[i % len(palette)]
            fig.add_trace(go.Scatter(x=labels, y=series(lm,d), name=d, legendgroup=d,
    ...
    layout = dict(barmode="group", height=480, hovermode="x unified",
        xaxis=dict(title="Departure (Travel) Month", domain=[0, 0.90] if n_right==2 else [0,1.0]),
    ...
    return fig, dests, len(order), line_specs
